...
# ---------------------------------------------------------------------------
# Multi-level Coruscant structures.
# ---------------------------------------------------------------------------
def platform(m, name, x1, x2, y1, y2, t="concrete2", ztop=GROUND_TOP,
             ambience=None, vol=-10):
    """A level deck floating at the current base_z. Cells outside it at this Z
    are open, so walking off the edge drops you toward the level below."""
    m.tile(x1, x2, y1, y2, 0, 0, t)
    m.zone(x1, x2, y1, y2, 0, ztop, name)
    if ambience:
        m.src(x1, x2, y1, y2, 0, ztop, ambience, vol)


def edge_railing(m, name, x1, x2, y1, y2, side, wtype="wallfence"):
    """A guard rail along one edge of a platform so you don't fall by accident.
    side in NSEW. Leaves the rest of the edge open."""
    if side == "N":
        m.tile(x1, x2, y2, y2, 0, 3, wtype); m.zone(x1, x2, y2 - 1, y2 - 1, 0, GROUND_TOP, name)
    elif side == "S":
        m.tile(x1, x2, y1, y1, 0, 3, wtype); m.zone(x1, x2, y1 + 1, y1 + 1, 0, GROUND_TOP, name)
    elif side == "E":
        m.tile(x2, x2, y1, y2, 0, 3, wtype); m.zone(x2 - 1, x2 - 1, y1, y2, 0, GROUND_TOP, name)
    elif side == "W":
        m.tile(x1, x1, y1, y2, 0, 3, wtype); m.zone(x1 + 1, x1 + 1, y1, y2, 0, GROUND_TOP, name)


def turbolift_tower(m, name, x1, x2, y1, y2, levels, exits=None,
                    wall_t="metal2", floor_t="metal5"):
    """A turbolift serving several levels. `levels` = list of (level_name,
    absolute_z). `exits` maps a level's z to the wall side (NSEW) whose doorway
    opens onto that level's platform (default 'S'). Builds an enclosed car with a
    control panel at every level and declares the turbolift directive covering
    the panel across all levels.
    Prerequisite: base_z must be 0 when called; it is restored to 0 on return."""
    exits = exits or {}
    zs = [z for _, z in levels]
    zmin, zmax = min(zs), max(zs)
    cx = (x1 + x2) // 2
    # Panel footprint: a two-tile console against the north interior wall.
    pax1, pax2 = cx - 1, cx
    pay1, pay2 = y2 - 2, y2 - 2
    for lname, lz in levels:
        m.at_level(lz)
        m.tile(x1, x2, y1, y2, 0, 0, floor_t)
        m.zone(x1, x2, y1, y2, 0, ROOM_TOP, name + " car (" + lname + ")")
        side = exits.get(lz, "S")
        gN = door_gap("N", x1, x2, y1, y2, 3) if side == "N" else None
        gS = door_gap("S", x1, x2, y1, y2, 3) if side == "S" else None
        gE = door_gap("E", x1, x2, y1, y2, 3) if side == "E" else None
        gW = door_gap("W", x1, x2, y1, y2, 3) if side == "W" else None
        m.wall_y(y2, x1, x2, ROOM_TOP, wall_t, [gN[1:]] if gN else None)
        m.wall_y(y1, x1, x2, ROOM_TOP, wall_t, [gS[1:]] if gS else None)
        m.wall_x(x1, y1, y2, ROOM_TOP, wall_t, [gW[1:]] if gW else None)
        m.wall_x(x2, y1, y2, ROOM_TOP, wall_t, [gE[1:]] if gE else None)
        m.tile(x1, x2, y1, y2, ROOM_TOP, ROOM_TOP, wall_t)   # car ceiling
        # the control console
        m.tile(pax1, pax2, pay1, pay2, 0, 3, "metal4")
        m.zone_raw(pax1, pax2, pay1, pay2, 0, GROUND_TOP, name + " control panel")
        # doorway label onto the platform
        if side == "N":
            m.zone_raw(gN[1], gN[2], y2, y2, 0, GROUND_TOP, name + " doors")
        elif side == "S":
            m.zone_raw(gS[1], gS[2], y1, y1, 0, GROUND_TOP, name + " doors")
        elif side == "E":
            m.zone_raw(x2, x2, gE[1], gE[2], 0, GROUND_TOP, name + " doors")
        elif side == "W":
            m.zone_raw(x1, x1, gW[1], gW[2], 0, GROUND_TOP, name + " doors")
        m.seed(cx, (y1 + y2) // 2, 0)
    m.at_level(0)
    # The directive: Shift+P works while standing on the panel at any level.
    m.turbolift(pax1, pax2, pay1, pay2, zmin, zmax + ROOM_TOP, name, levels)


def hole(m, x1, x2, y1, y2, label):
    """Punch a deliberate fall-through opening in the current platform (its floor
    is removed so you drop to the level below). The label carries the 'open
    shaft' marker the validator uses to skip it."""
    m.tile(x1, x2, y1, y2, 0, 0, "blank")
    m.zone(x1, x2, y1, y2, 0, GROUND_TOP, label + " (open shaft to the level below)")
