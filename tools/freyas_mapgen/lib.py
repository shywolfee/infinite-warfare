...
class Map:
    ...
    # ---- transition (absolute Z) ------------------------------------------
    def transition(self, x1, x2, y1, y2, z1, z2, dx, dy, dz, label):
        """An interaction point: standing inside x1..x2,y1..y2,z1..z2 and
        pressing Shift+L moves the player to (dx,dy,dz). All coordinates are
        absolute. The label is spoken on use; it must not contain a colon."""
        self.lines.append("transition:%d:%d:%d:%d:%d:%d:%d:%d:%d:%s"
                          % (x1, x2, y1, y2, z1, z2, dx, dy, dz, label))
